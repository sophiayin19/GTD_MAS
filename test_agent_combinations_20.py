...
def test_agent_combinations():
    ...
    agent_combinations = [
        # Single agents
        (["Programming_Expert"], "4", 4, "4 Programming Experts"),
        (["Algorithm_Designer"], "4", 4, "4 Algorithm Designers"),
        (["Test_Analyst"], "4", 4, "4 Test Analysts"),
        (["Bug_Fixer"], "4", 4, "4 Bug Fixers"),
        ...
        # Two-agent combinations with multiple agents
        (["Programming_Expert", "Algorithm_Designer"], "2 2", 4, "2 Programming Experts + 2 Algorithm Designers"),
        (["Programming_Expert", "Test_Analyst"], "2 2", 4, "2 Programming Experts + 2 Test Analysts"),
    ...
    print("Testing on 20 problems each to find the best combination...")
    print()
    ...
    for i, (agent_names, agent_nums, agent_count, description) in enumerate(agent_combinations, 1):
        ...
        # Create test dataset with 20 problems instead of 10
        test_file = f"datasets/humaneval/humaneval-test-{i}.jsonl"
        subprocess.run(f"head -20 datasets/humaneval/humaneval-py.jsonl > {test_file}", shell=True)
        ...
        try:
            result = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=600)  # Increased timeout for 20 problems
            end_time = time.time()
